...
开源 Community Edition：面试准备 skills、岗位知识库、工作台数据契约、工作台取数参考服务和虚构示例工作台。**本仓库不含产品界面，也不含任何靠模型跑起来的能力**：实时提词、模拟面试接口、一次生成工作台都在官方产品 [greenroom.ungetsu.net](https://greenroom.ungetsu.net/)，它的前端、账号同步、托管服务端和品牌资产同样不在本仓库。结构见 [README.md](README.md)，数据契约见 [docs/workspace-spec.md](docs/workspace-spec.md)。许可证为 AGPL-3.0；品牌使用见 [TRADEMARK.md](TRADEMARK.md)。
...
- **服务直连契约**：`serve.py` 与 `docs/workspace-spec.md`「工具取数约定」的 `/workspace/bundle`、`/workspace/file` 两端点保持一致。这两个端点是第三方工具的对接面，签名不能随手改；`serve.py` 只做取数、`/config` 与 `/api/resolve-logo`（取公开图片直链，不调模型），不要往里加任何调模型的端点。
...
- `./start.sh ~/some-workspace` 起本地取数服务，`curl http://127.0.0.1:8765/workspace/bundle` 能拿到工作台全文

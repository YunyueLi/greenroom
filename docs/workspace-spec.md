...
本契约定义的是目录和文件格式，不定义网络接口。兼容工具必须由使用者显式选择工作台目录或具体文件，再在已经授权的范围内读取；不得在后台探测回环地址、扫描本机端口或启动常驻服务。

- 命令行和桌面工具可以直接读取使用者指定的目录，并按本文的相对路径解析 Markdown 与附件。
- 浏览器工具应使用文件选择、目录选择或拖放；如果浏览器允许持久化目录句柄，也必须清楚显示已授权范围，并允许使用者随时断开。
- 读取实现必须拒绝越出工作台根目录的路径，未知文件默认不执行，只按文本或附件处理。
- 未经使用者明确知情和同意，不得把工作台内容上传到第三方服务。

最小的直接读盘示例见 `docs/realtime-bridge.md`。本仓库不提供 Greenroom 本地 HTTP 运行时，也不把任何回环端口作为产品或兼容性契约。

## 隐私约定

工作台目录**永远不要**放进公开仓库。本仓库只含 skills、岗位知识库、契约文档、文件格式工具和虚构示例（`examples/demo-workspace/`），不含任何真实个人数据。
